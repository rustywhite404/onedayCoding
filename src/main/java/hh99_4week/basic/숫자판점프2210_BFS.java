...
public class 숫자판점프2210_BFS {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //BFS 시작. 숫자판의 모든 위치(5x5)에서 한 번씩 실행해야 하므로 아래와 같이 반복
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
    ...
    private static void bfs(int startX, int startY) {
        Queue<int[]> positionQueue = new LinkedList<>(); // {x,y 좌표저장}
        Queue<String> routeQueue = new LinkedList<>(); // 경로 문자열 저장

        positionQueue.add(new int[]{startX, startY, 0}); //초기값 : {x, y, count}
        routeQueue.add(numMap[startX][startY]); //초기값 : 경로
        //큐 하나로 풀어도 되지만 좀 더 명확하게 값들을 확인하려고 분리했다. 좌표 정보와 경로 정보로 분리.

        while (!positionQueue.isEmpty()){
            //현재 상태 하나 꺼내오기
            int[] current = positionQueue.poll();
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (0 <= nowX && nowX < 5 && 0 <= nowY && nowY < 5) { //숫자판을 벗어나지 않도록 0~4 사이인지 검사. 같은 칸을 중복으로 밟아도 되므로 visited 체크하지 않음
                    positionQueue.add(new int[]{nowX, nowY, count+1});
                    routeQueue.add(route+numMap[nowX][nowY]); //경로 초기값에 현재 위치값도 더해준다(=> 이것때문에 count+1이 됨)
                }
            }
        }
